...
| `<results-dir>/`     | produced | One per run: responses, raw event streams, scores, masked review copies       |
...
and vocabulary still identify an arm to a judge that is looking.

`judge-agy.sh` records `ERROR` when the model returns no structured output, never a zero. A zero
would pull that arm's mean down and look like data.

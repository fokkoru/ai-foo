...
Step 3 is **required**, not optional: Codex plugins can only bundle skills, so the 7 subagents that `research`/`planning`/`iterate`/`peer-review` spawn must be copied into `~/.codex/agents/` separately. The script copies all nine — those 7 plus the two advisors, which no skill spawns and you invoke yourself (and it reminds you to enable `web_search` for `web-search-researcher`).

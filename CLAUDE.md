...
- `${CLAUDE_PLUGIN_ROOT}` is **not** substituted in a SKILL.md body. Claude Code substitutes it for hooks, agents, commands and MCP configs; a skill body gets `${CLAUDE_SKILL_DIR}` and `${CLAUDE_SESSION_ID}` and nothing else — observed on Claude Code 2.1.261. An unsubstituted `${CLAUDE_PLUGIN_ROOT}` is not an error: it reaches the model as literal text and the path it names does not exist. Refer to a bundled file by a path relative to the skill's own directory instead; `CONTRIBUTING.md` carries how that resolves on each runtime.

...
Stop and report what you have if:

- Answering needs files beyond the ones the request names and their direct dependencies — the question is not narrow, which is a fact about the request rather than about your progress. Say so and recommend `df:architecture-advisor` instead of widening the read.
- The request contains more than one decision.
- Codex returns contradictory recommendations across a thread.
- Three exchanges on one thread without converging.

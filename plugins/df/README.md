...
| `df:implement`   | Every wave ends with one `code-reviewer` pass over that wave's diff, a run of two or more waves ends with one more over the whole run, each blocking finding goes to a `finding-verifier` before it is fixed, the plan's repo-wide commands run once for the run rather than once per phase, and a plan carrying frontmatter reads `status: implemented` when the run ends |

...
| `df:implement`   | Execute plans one phase at a time in the main thread, verifying and reviewing each phase before the next                                                              |
...
| `df:planning`    | A new file exists under `thoughts/plans/`, and it opens with a frontmatter block whose `status` is `draft`                                                                                                                                                                                                                                                                                                          |
| `df:iterate`     | The plan file changed, and every phase it still names is numbered exactly once                                                                                                                                                                                                                                                                                                                                      |
| `df:implement`   | Every phase ends with one `code-reviewer` pass over that phase's diff, a run of two or more phases ends with one more over the whole run, each spec gap and each Critical or Important finding goes to a `finding-verifier` before it is fixed, the plan's `### Acceptance Criteria` run once for the run rather than once per phase, and a plan carrying frontmatter reads `status: implemented` when the run ends |

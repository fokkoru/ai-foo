...
| `kb:weave`                        | `docs/` changed and `thoughts/` did not — the run reports a source-hash verification, a second document on a topic updated an existing page instead of adding a sibling, and every non-capture source it read is recorded in `docs/kb-intake.tsv` at its current hash                                                                                      |
| `kb:weave` on a hand-written tree | whichever of `docs/WIKI.md`, `docs/index.md` and `docs/log.md` were missing appeared, every page without a `type:` line gained one — and a decision page without an id a `decision_id:` line — and nothing else in its diff, `thoughts/` did not change, nothing was compiled, and the run showed you the directory-to-type table before it wrote anything |

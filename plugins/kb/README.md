...
The raw corpus under `thoughts/` is the input and is never written to. The compiled layer under `docs/` is the output: committed, readable by a person, and shaped by Open Knowledge Format v0.2, whose page layout and footnote-as-join-key it follows. The compiler's own state — `.kb/schema.md`, and a fingerprint and a provenance row per compiled page — lives in `.kb/`, hidden and tracked alongside `docs/`. Every compiled page's footnotes are backed by a `.kb/provenance.tsv` row naming a hash of the exact fragment it was built from — a field the format does not define and this plugin adds — so drift between a page and its source is something a script reports rather than something you discover by reading.
...
| Skill        | Description                                                                                                                       |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------- |
| `kb:weave`   | Weave the raw corpus under `thoughts/` into a knowledge base in `docs/`, with its compiler state in `.kb/`                        |
